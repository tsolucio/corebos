...
function main__asteriskClient() {
	...
	while (true) {
		...
		sleep(1);
		try {
			asterisk_handleEvents($asterisk, $adb, $data['version']);
...
function asterisk_handleResponse3($mainresponse, $adb, $asterisk) {
	...
	if ($mainresponse['Event'] == 'Link' || ($mainresponse['Event'] == 'Bridge' && $mainresponse['Bridgestate'] == 'Link')) {
		...
		$checkres = $adb->pquery(
			'SELECT * FROM vtiger_asteriskincomingevents WHERE uid=? and (flag = 0 or flag = -1) and (from_number is NULL or from_number = 0)',
			array($uid)
		);
