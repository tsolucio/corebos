...
var fieldValueArr=new Array('Text', 'Number', 'Percent', 'Currency', 'Date', 'Email', 'Phone', 'Picklist', 'URL', 'Checkbox', 'TextArea', 'MultiSelectCombo', 'Skype', 'Time', 'Relation', 'Image', 'Datetime');
var fieldTypeArr=new Array('text', 'number', 'percent', 'currency', 'date', 'email', 'phone', 'picklist', 'url', 'checkbox', 'textarea', 'multiselectcombo', 'skype', 'time', 'relation', 'image', 'datetime');
...
function selFieldType(id, scrollLayer, bool, blockid) {
	...
	if (type=='text') {
		...
		if (relationmodules!=undefined) {
			relationmodules.style.display='none';
		}
	} else if (type=='date' || type=='percent' || type=='email' || type=='phone' || type=='url' || type=='checkbox' || type=='textarea' || type=='skype' || type=='time' || type=='image' || type=='datetime') {
	...
	if (doSearch==true) {
		...
		default : //Character Search
			if (keyCode>=65 && keyCode<=90) {
				...
				var loop=1;
				for (var i=startIdx; i<=totFieldType;) {
					var currFieldStr=fieldLayer.getObj('field'+i).innerHTML;
...
function cloneAndAddLeadMapping(nodeid, tableid) {
	...
	var snoHTML=document.getElementById('snoDiv').innerHTML;
	...
	var leadHTML=document.getElementById('leadCloneDiv').innerHTML;
	...
	var accHTML=document.getElementById('accountCloneDiv').innerHTML;
	...
	var conHTML=document.getElementById('contactCloneDiv').innerHTML;
	...
	var potHTML=document.getElementById('potentialCloneDiv').innerHTML;
...
function validateMapping(id, chngSelect, field_id) {
	var alertmessage = new Array(alert_arr.LBL_TYPEALERT_1, alert_arr.LBL_WITH, alert_arr.LBL_TYPEALERT_2, alert_arr.LBL_LENGTHALERT, alert_arr.LBL_DECIMALALERT, alert_arr.LBL_MAPPEDALERT);
	var leadf=document.getElementById('map['+id+'][Leads]');
	var accountf=document.getElementById('map['+id+'][Accounts]');
	var contactf=document.getElementById('map['+id+'][Contacts]');
	var potentialf=document.getElementById('map['+id+'][Potentials]');

	var leadDataType=leadf.options[leadf.selectedIndex].getAttribute('typeofdata');
	var accountDataType=accountf.options[accountf.selectedIndex].getAttribute('typeofdata');
	var contactDataType=contactf.options[contactf.selectedIndex].getAttribute('typeofdata');
	var potentialDataType=potentialf.options[potentialf.selectedIndex].getAttribute('typeofdata');

	var leadType=leadf.options[leadf.selectedIndex].getAttribute('fieldtype');
	var accountType=accountf.options[accountf.selectedIndex].getAttribute('fieldtype');
	var contactType=contactf.options[contactf.selectedIndex].getAttribute('fieldtype');
	var potentialType=potentialf.options[potentialf.selectedIndex].getAttribute('fieldtype');
