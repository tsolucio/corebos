...
Array (
	...
	public function buildSelectStmt($sqlDump) {
		...
		if (in_array('*', $sqlDump['column_list'])) {
			...
			$i=0;
			foreach ($sqlDump['column_list'] as $field) {
		...
		if (!empty($sqlDump['where_condition'])) {
			...
			if ($sofCN == $sofCV && $sofCO == ($sofOp+1)) {
				...
				for ($i=0; $i < count($sqlDump['where_condition']['column_values']); ++$i) {
					if (!$fieldcol[$sqlDump['where_condition']['column_names'][$i]]) {
						throw new WebServiceException(
							WebServiceErrorCode::$ACCESSDENIED,
							'Permission to access '.$sqlDump['where_condition']['column_names'][$i].' attribute denied.'
						);
					}
		...
		if (!empty($sqlDump['limit'])) {
			$i=0;
			$offset = (count($sqlDump['limit']) > 1);
			$this->query = $this->query.' LIMIT ';
			foreach ($sqlDump['limit'] as $field) {
	...
	public function yy_find_shift_action($iLookAhead) {
		...
		if ($i < 0 || $i >= self::YY_SZ_ACTTAB ||
			...
			if (count(self::$yyFallback) && $iLookAhead < count(self::$yyFallback) && ($iFallback = self::$yyFallback[$iLookAhead]) != 0) {
				if (self::$yyTraceFILE) {
					fwrite(self::$yyTraceFILE, self::$yyTracePrompt . 'FALLBACK ' . self::$yyTokenName[$iLookAhead] . ' => ' . self::$yyTokenName[$iFallback] . "\n");
				}
	...
	public function yy_r41() {
		if (empty($this->out['meta'])) {
			...
			if (in_array('*', $this->out['column_list'])) {
				...
				foreach ($this->out['column_list'] as $field) {
			...
			if (!empty($this->out['where_condition'])) {
				foreach ($this->out['where_condition']['column_names'] as $field) {
					$columns[] = $fieldcol[$field];
				}
			}
			...
			foreach ($tables as $table) {
				if ($firstTable!=$table) {
					if (!isset($tabNameIndex[$table]) && $table == 'vtiger_crmentity') {
						...
						$table = $module != 'Documents' ? $mod::$crmentityTable : 'vtiger_crmentity';
