...
function cbws_getQueuedOperationResult($operationTrackingID, $user) {
	if (coreBOS_Settings::settingExists($operationTrackingID)) {
		$return = coreBOS_Settings::getSetting($operationTrackingID, '');
		coreBOS_Settings::delSetting($operationTrackingID);
		return $return;
	} else {
		return 'Operation not queued or not finished yet';
	}
}