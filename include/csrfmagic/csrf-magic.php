 ...
 */
$GLOBALS['csrf']['rewrite-js'] = 'include/csrfmagic/csrf-magic.js';
 ...
 */
$GLOBALS['csrf']['input-name'] = '__vt5rftk';
...
function csrf_check($fatal = true) {
	global $site_URL;
	if ((strpos($site_URL, $_SERVER['HTTP_HOST']) !== false) && (empty($_SERVER['REQUEST_METHOD']) || $_SERVER['REQUEST_METHOD'] !== 'POST')) {
		return true;
	}
	...
	if (empty($_POST[$name])) {
		...
		if (!empty($request_body) && strpos($request_body, '=')) {
			parse_str($request_body, $requestPayload);
			if (!empty($requestPayload[$name])) {
				$_POST[$name] = $requestPayload[$name];
			}
		}
	}
	do {
...
function csrf_check_token($token) {
	...
	switch ($type) {
		case 'sid':
			return $value === csrf_hash(coreBOS_Session::getSessionName(), $time);
		case 'cookie':
 ...
 */
function csrf_start() {
	if ($GLOBALS['csrf']['auto-session'] && !coreBOS_Session::isSessionStarted()) {
