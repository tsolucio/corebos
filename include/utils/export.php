...
function export($type, $format = 'CSV') {
	...
	if ($format == 'CSV') {
		...
		echo $header;
		$dumprows = dumpRowsToCSV($type, $__processor, $CSV_Separator, $columnsToExport, $result, $focus);
	}
	if ($format == 'XLS') {
		$dumprows = dumpRowsToXLS($__processor, $columnsToExport, $translated_fields_array, $result);
	}
...
if (isset($_REQUEST['exportfile']) && $_REQUEST['exportfile']=='exportexcelfile') {
	...
	$totalxclinfo = array();
	$fieldinfo = array();
	$fldname = $_REQUEST['module'].' Excel Format';
	$workbook = exportExcelFileRows(export(vtlib_purify($_REQUEST['module']), 'XLS'), $totalxclinfo, $fldname, $fieldinfo, $_REQUEST['module']);
