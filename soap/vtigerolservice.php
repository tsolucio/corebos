...
$server->register(
	'AddEmailAttachment',
	array(
		'emailid'=>'xsd:string','filedata'=>'xsd:string',
...
function AddMessageToContact($username, $session, $contactid, $msgdtls) {
	...
	foreach ($msgdtls as $msgdtl) {
		if (isset($msgdtl)) {
			...
			$email->column_fields['date_start'] = $date_sent->format('Y-m-d');
...
function LoginToVtiger($user_name, $password, $version) {
	...
	if ($password != '') {
		...
		if ($objuser->is_authenticated()) {
			...
			$adb->pquery('insert into vtiger_soapservice values(?,?,?)', array($userid, 'Outlook', $sessionid));
...
function AddEmailAttachment($emailid, $filedata, $filename, $filesize, $filetype, $username, $session) {
	...
	if ($entityresult != false) {
		...
		$adb->pquery('insert into vtiger_seattachmentsrel values(?,?)', array($emailid, $crmid));
...
function AddContacts($username, $session, $cntdtls) {
	...
	if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
		...
		$sql1 = 'select fieldname,columnname
			from vtiger_field
...
function retrieve_account_id($account_name, $user_id) {
	...
	$query = 'select vtiger_account.accountname accountname,vtiger_account.accountid accountid
		from vtiger_account
	...
	if (!$result || $db->num_rows($result)==0) {
		die('Not able to retreive account information');
	}
...
function GetTasks($username, $session) {
	...
	while ($task = $adb->fetch_array($result)) {
		...
		if ($task["status"] == "Not Started") {
			...
			$task["status"] = "4";
		} elseif ($task["status"] == "Pending Input" || $task["status"] == "Planned") {
			$task["status"] = "3";
		} else {
			$task["status"] = "0";
		}
...
function AddTasks($username, $session, $taskdtls) {
	...
	if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
		...
		$sql1 = 'select fieldname,columnname
			...
			inner join vtiger_profile2field on vtiger_profile2field.fieldid=vtiger_field.fieldid
			inner join vtiger_def_org_field on vtiger_def_org_field.fieldid=vtiger_field.fieldid
			where vtiger_field.tabid=9 and vtiger_field.displaytype in (1,2,4,3) and vtiger_profile2field.visible=0
				and vtiger_def_org_field.visible=0 and vtiger_field.presence in (0,2)';
		$params1 = array();
		if (count($profileList) > 0) {
			$sql1 .= " and vtiger_profile2field.profileid in (". generateQuestionMarks($profileList) .")";
	...
	//Currently only 3 status avail Note ************************************************
		if (isset($taskrow)) {
			...
			} elseif ($taskrow["status"] == "4") {
				$taskrow["status"] = "Deferred";
			} elseif ($taskrow["status"] == "3") {
			...
			} elseif ($taskrow["priority"] == "0") {
				$taskrow["priority"] = "Low";
			} elseif ($taskrow["priority"] == "1") {
...
function AddClndr($username, $session, $clndrdtls) {
	...
	if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
		...
		$sql1 = 'select fieldname,columnname
			...
			inner join vtiger_profile2field on vtiger_profile2field.fieldid=vtiger_field.fieldid
			inner join vtiger_def_org_field on vtiger_def_org_field.fieldid=vtiger_field.fieldid
			where vtiger_field.tabid=16 and vtiger_field.displaytype in (1,2,4,3) and vtiger_profile2field.visible=0
				and vtiger_def_org_field.visible=0 and vtiger_field.presence in (0,2)';
		$params1 = array();
		if (count($profileList) > 0) {
			$sql1 .= " and vtiger_profile2field.profileid in (". generateQuestionMarks($profileList) .")";
