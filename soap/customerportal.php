 ...
 * @return array $response - ticket comments and details as array with elements comments, owner and createdtime. returned from the function get_ticket_comments_list
*/
...
function get_combo_values($input_array) {
	...
	$result1 = $adb->pquery(
		"select vtiger_ticketpriorities.ticketpriorities
			from vtiger_ticketpriorities
			inner join vtiger_role2picklist on vtiger_role2picklist.picklistvalueid = vtiger_ticketpriorities.picklist_valueid and vtiger_role2picklist.roleid='$admin_role'
			order by sortid",
		array()
	);
	...
	$result2 = $adb->pquery(
		"select vtiger_ticketseverities.ticketseverities
			from vtiger_ticketseverities
			inner join vtiger_role2picklist on vtiger_role2picklist.picklistvalueid = vtiger_ticketseverities.picklist_valueid and vtiger_role2picklist.roleid='$admin_role'
			order by sortid",
		array()
	);
	...
	$result3 = $adb->pquery(
		"select vtiger_ticketcategories.ticketcategories
			from vtiger_ticketcategories
			inner join vtiger_role2picklist on vtiger_role2picklist.picklistvalueid = vtiger_ticketcategories.picklist_valueid and vtiger_role2picklist.roleid='$admin_role'
			order by sortid",
		array()
	);
...
function get_KBase_details($input_array) {
	...
	$category_query = "select vtiger_faqcategories.faqcategories
		from vtiger_faqcategories
		inner join vtiger_role2picklist on vtiger_role2picklist.picklistvalueid = vtiger_faqcategories.picklist_valueid and vtiger_role2picklist.roleid='$admin_role'";
	...
	if ($check == true) {
		$product_query = 'select productid, productname
			from vtiger_products
			inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_products.productid
			where vtiger_crmentity.deleted=0';
...
function save_faq_comment($input_array) {
	...
	if (trim($comment) != '') {
		$adb->pquery('insert into vtiger_faqcomments values(?,?,?,?)', array('', $faqid, $comment, $createdtime));
	}
 ...
 *	@param string $login - true or false.
 *		If true means function has been called for login process and we have to clear the session if any,
 *		false means not called during login and we should not unset the previous sessions
...
function authenticate_user($username, $password, $version, $login = 'true') {
	...
	if ($num_rows > 1) {
		return $err[0];//More than one user
	} elseif ($num_rows <= 0) {
	...
	if ($login != 'false') {
		...
		$adb->pquery('insert into vtiger_soapservice values(?,?,?)', array($customerid, 'customer', $sessionid));
...
function change_password($input_array) {
	...
	$adb->pquery('update vtiger_portalinfo set user_password=? where id=? and user_name=?', array($password, $id, $username));
...
function get_picklists($input_array) {
	...
	$res = $adb->pquery(
		"select vtiger_". $picklist_name.".* from vtiger_". $picklist_name
			." inner join vtiger_role2picklist on vtiger_role2picklist.picklistvalueid = vtiger_". $picklist_name.".picklist_valueid and vtiger_role2picklist.roleid='$admin_role'",
		array()
	);
...
function add_ticket_attachment($input_array) {
	...
	$adb->pquery($crmquery, array($attachmentid, 'HelpDesk Attachment', $description, $date_var));

	$attachmentquery = 'insert into vtiger_attachments(attachmentsid,name,description,type,path) values(?,?,?,?,?)';
	$adb->pquery($attachmentquery, array($attachmentid, $filename, $description, $filetype, $upload_filepath));

	$adb->pquery('insert into vtiger_seattachmentsrel values(?,?)', array($ticketid, $attachmentid));
...
function getServerSessionId($id) {
	...
	if ($sessionid === false) {
		$result = $adb->pquery("select * from vtiger_soapservice where type='customer' and id=?", array($id));
...
function get_list_values($id, $module, $sessionid, $only_mine = 'true') {
	...
	if ($module == 'Quotes') {
		...
		where vtiger_crmentity.deleted=0 and (vtiger_quotes.accountid in (".generateQuestionMarks($entity_ids_list)
			.') or contactid in ('. generateQuestionMarks($entity_ids_list) .'))';
		...
		foreach ($fields_list as $fieldlabel => $fieldname) {
			...
			if ($module == 'Documents') {
				...
				if ($fieldname == 'filename') {
					...
					if ($fileactive == 1) {
						if ($filetype == 'I') {
							$fieldvalue = '<a href="index.php?&downloadfile=true&folderid='.$folderid.'&filename='.$filename.'&module=Documents&action=index&id='.$fieldid
								.'">'.$fieldvalue.'</a>';
						} elseif ($filetype == 'E') {
...
function get_filecontent_detail($id, $folderid, $module, $customerid, $sessionid) {
	...
	if ($module == 'Documents') {
		...
		$query ='select vtiger_attachments.*,vtiger_seattachmentsrel.*
...
function updateCount($id) {
	global $log;
	$log->debug('Entering/Exiting customer portal function updateCount');
	return updateDownloadCount($id);
}
...
function updateDownloadCount($id) {
	global $adb,$log;
	$log->debug('Entering/Exiting customer portal function updateDownloadCount');
	$adb->pquery('UPDATE vtiger_notes SET filedownloadcount = filedownloadcount+1 WHERE notesid = ?', array($id));
	return true;
}
...
function get_product_list_values($id, $modulename, $sessionid, $only_mine = 'true') {
	...
	if ($checkQuotes == true) {
		$query[] = "select distinct vtiger_products.*,
			...
			where vtiger_inventoryproductrel.productid = vtiger_products.productid AND vtiger_crmentity.deleted=0 and (accountid in  ("
				.generateQuestionMarks($entity_ids_list) .') or contactid in ('. generateQuestionMarks($entity_ids_list) .'))';
		$params[] = array($entity_ids_list,$entity_ids_list);
	}
...
function get_details($id, $module, $customerid, $sessionid) {
	...
	$fieldquery = 'SELECT fieldname,columnname,fieldlabel,blocklabel,uitype
		FROM vtiger_field
		INNER JOIN  vtiger_blocks on vtiger_blocks.blockid=vtiger_field.block
		WHERE vtiger_field.tabid = ? AND displaytype in (1,2,4)
	...
	for ($i=0; $i<$nooffields; $i++) {
		...
		if ($blocklabel == 'LBL_COMMENTS' || $blocklabel == 'LBL_IMAGE_INFORMATION') { // comments block of tickets is hardcoded in customer portal > get_ticket_comments
			continue;
		}
...
function check_permission($customerid, $module, $entityid) {
	...
	switch ($module) {
		case 'Products':
			$query = 'SELECT vtiger_seproductsrel.productid
				FROM vtiger_seproductsrel
				INNER JOIN vtiger_crmentity ON vtiger_seproductsrel.productid=vtiger_crmentity.crmid
				WHERE vtiger_seproductsrel.crmid IN ('. generateQuestionMarks($allowed_contacts_and_accounts)
					.') AND vtiger_crmentity.deleted=0 AND vtiger_seproductsrel.productid = ?';
			...
			$query = "SELECT vtiger_inventoryproductrel.productid, vtiger_inventoryproductrel.id
				...
				INNER JOIN vtiger_crmentity ON vtiger_inventoryproductrel.productid=vtiger_crmentity.crmid
				LEFT JOIN vtiger_quotes ON vtiger_inventoryproductrel.id = vtiger_quotes.quoteid
				WHERE vtiger_crmentity.deleted=0 AND (vtiger_quotes.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts)
					.') or vtiger_quotes.accountid IN ('.generateQuestionMarks($allowed_contacts_and_accounts).')) AND vtiger_inventoryproductrel.productid = ?';
			...
			$query = 'SELECT vtiger_inventoryproductrel.productid, vtiger_inventoryproductrel.id
				FROM vtiger_inventoryproductrel
				INNER JOIN vtiger_crmentity ON vtiger_inventoryproductrel.productid=vtiger_crmentity.crmid
				LEFT JOIN vtiger_invoice ON vtiger_inventoryproductrel.id = vtiger_invoice.invoiceid
				WHERE vtiger_crmentity.deleted=0 AND (vtiger_invoice.contactid IN ('. generateQuestionMarks($allowed_contacts_and_accounts)
					.') or vtiger_invoice.accountid IN ('.generateQuestionMarks($allowed_contacts_and_accounts).')) AND vtiger_inventoryproductrel.productid = ?';
			$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
			if ($adb->num_rows($res) > 0) {
				return true;
			}
		...
		case 'Quotes':
			$query = 'SELECT vtiger_quotes.quoteid
				...
				WHERE vtiger_crmentity.deleted=0 AND (vtiger_quotes.contactid IN ('. generateQuestionMarks($allowed_contacts_and_accounts)
					.') or vtiger_quotes.accountid IN ('.generateQuestionMarks($allowed_contacts_and_accounts).')) AND vtiger_quotes.quoteid = ?';
			$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
			if ($adb->num_rows($res) > 0) {
				return true;
			}
		...
		case 'Invoice':
			$query = 'SELECT vtiger_invoice.invoiceid
				...
				WHERE vtiger_crmentity.deleted=0 AND (vtiger_invoice.contactid IN ('. generateQuestionMarks($allowed_contacts_and_accounts)
					.') or vtiger_invoice.accountid IN ('.generateQuestionMarks($allowed_contacts_and_accounts).')) AND vtiger_invoice.invoiceid = ?';
			$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
			if ($adb->num_rows($res) > 0) {
				return true;
			}
		...
		case 'Documents':
			$query = 'SELECT vtiger_senotesrel.notesid FROM vtiger_senotesrel
				...
				WHERE vtiger_senotesrel.crmid IN ('. generateQuestionMarks($allowed_contacts_and_accounts) .') AND vtiger_senotesrel.notesid = ?';
			$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
			if ($adb->num_rows($res) > 0) {
				return true;
			}
			if (checkModuleActive('Project')) {
				$query = 'SELECT vtiger_senotesrel.notesid
					...
					WHERE vtiger_project.linktoaccountscontacts IN ('. generateQuestionMarks($allowed_contacts_and_accounts) .') AND vtiger_senotesrel.notesid = ?';
				$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
				if ($adb->num_rows($res) > 0) {
					return true;
				}
			...
			$query = 'SELECT vtiger_senotesrel.notesid
				FROM vtiger_senotesrel
				INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_senotesrel.notesid AND vtiger_crmentity.deleted = 0
				WHERE vtiger_senotesrel.crmid IN ('. generateQuestionMarks($faq_id) .') AND vtiger_senotesrel.notesid = ?';
		...
		case 'HelpDesk':
			$query = 'SELECT vtiger_troubletickets.ticketid
				FROM vtiger_troubletickets
				INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_troubletickets.ticketid AND vtiger_crmentity.deleted = 0
				WHERE vtiger_troubletickets.parent_id IN ('. generateQuestionMarks($allowed_contacts_and_accounts) .') AND vtiger_troubletickets.ticketid = ?';
			$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
			if ($adb->num_rows($res) > 0) {
				return true;
			}
			...
			$query = 'SELECT vtiger_troubletickets.ticketid
				...
				WHERE vtiger_crmentity.deleted = 0 AND (vtiger_crmentityrel.crmid IN (SELECT projectid FROM vtiger_project WHERE linktoaccountscontacts IN ('
					.generateQuestionMarks($allowed_contacts_and_accounts)
					.')) OR vtiger_crmentityrel.relcrmid IN (SELECT projectid FROM vtiger_project WHERE linktoaccountscontacts IN ('
					.generateQuestionMarks($allowed_contacts_and_accounts) .')) AND vtiger_troubletickets.ticketid = ?)';
			$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
			if ($adb->num_rows($res) > 0) {
				return true;
			}
		...
		case 'Assets':
			$query = 'SELECT vtiger_assets.assetname
				FROM vtiger_assets
				INNER JOIN vtiger_crmentity ON  vtiger_assets.assetsid = vtiger_crmentity.crmid
				WHERE vtiger_crmentity.deleted = 0 and vtiger_assets.account = ?';
			$accountid = '';
			$accountRes = $adb->pquery('SELECT accountid
				FROM vtiger_contactdetails
				INNER JOIN vtiger_crmentity ON vtiger_contactdetails.contactid = vtiger_crmentity.crmid
		...
		case 'Project':
			$query = 'SELECT vtiger_project.projectid
				FROM vtiger_project
				INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_project.projectid AND vtiger_crmentity.deleted = 0
				WHERE vtiger_project.linktoaccountscontacts IN ('. generateQuestionMarks($allowed_contacts_and_accounts) .') AND vtiger_project.projectid = ?';
			$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
			if ($adb->num_rows($res) > 0) {
				return true;
			}
...
function get_service_list_values($id, $modulename, $sessionid, $only_mine = 'true') {
	...
	if ($checkQuotes == true) {
		$query[] = "select distinct vtiger_service.*,
			...
			where vtiger_inventoryproductrel.productid = vtiger_service.serviceid AND vtiger_crmentity.deleted=0 and (accountid in  ("
				.generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) .'))';
		$params[] = array($entity_ids_list,$entity_ids_list);
	}
	$checkInvoices = checkModuleActive('Invoice');
...
function get_modules() {
	...
	if ($modules === false) {
		...
		$query = $adb->pquery(
			'SELECT vtiger_customerportal_tabs.*
