...
class Vtiger_PackageExport {
	private $_export_tmpdir = 'cache';
	private $_export_modulexml_filename = null;
	private $_export_modulexml_file = null;
	 ...
	 * @access private
	 */
	private function __initExport($module, $moduleInstance) {
	 ...
	 * @access private
	 */
	private function __finishExport() {
	 ...
	 * @access private
	 */
	private function __cleanupExport() {
		if (!empty($this->_export_modulexml_filename)) {
	...
	public static function packageFromFilesystem($moduleName, $mandatory = false, $directDownload = false) {
		...
		$wehavefiles = $wehavefiles && file_exists("modules/$moduleName/manifest.xml");  // check for manifest
		$wehavefiles = $wehavefiles && is_dir("modules/$moduleName/language");  // check for language directory
		if ($wehavefiles) {
			// Export as Zip
	...
	public static function languageFromFilesystem($languageCode, $languageName, $directDownload = false) {
		// first we check for the files
		$wehavefiles = file_exists("include/language/$languageCode.manifest.xml");  // check for manifest
		if ($wehavefiles && preg_match('/^[a-z]{2}_[a-z]{2}$/', $languageCode)) {
	 ...
	 * @access private
	 */
	private function export_Dependencies($moduleInstance) {
	 ...
	 * @access private
	 */
	private function export_Module($moduleInstance) {
		global $adb;
	 ...
	 * @access private
	 */
	private function export_Tables($moduleInstance) {
	 ...
	 * @access private
	 */
	private function export_Blocks($moduleInstance) {
		global $adb;
	 ...
	 * @access private
	 */
	private function export_Fields($moduleInstance, $blockid) {
		global $adb;
	 ...
	 * @access private
	 */
	private function export_CustomViews($moduleInstance) {
		global $adb;
	 ...
	 * @access private
	 */
	private function export_SharingAccess($moduleInstance) {
		global $adb;
	 ...
	 * @access private
	 */
	private function export_Events($moduleInstance) {
	 ...
	 * @access private
	 */
	private function export_Actions($moduleInstance) {
	 ...
	 * @access private
	 */
	private function export_RelatedLists($moduleInstance) {
	 ...
	 * @access private
	 */
	private function export_CustomLinks($moduleInstance) {
	 ...
	 * @access private
	 */
	private function export_CronTasks($moduleInstance) {
