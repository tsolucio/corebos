...
CreateMySQLUser()
{
    ...
    mysql -uroot -p${MYSQL_PASS} -e "ALTER USER '${MYSQL_USER}'@'localhost' IDENTIFIED BY '${MYSQL_PASS}'"
    mysql -uroot -p${MYSQL_PASS} -e "GRANT ALL PRIVILEGES ON *.* TO '${MYSQL_USER}'@'localhost' WITH GRANT OPTION"
...
OnCreateDB()
{
    if [ "$ON_CREATE_DB" = "**False**" ]; then
        ...
        mysql -uroot -p${MYSQL_PASS} -e "CREATE DATABASE IF NOT EXISTS ${ON_CREATE_DB};"
        mysql -uroot -p${MYSQL_PASS} corebostest </var/www/html/build/coreBOSTests/database/coreBOSTests.sql
