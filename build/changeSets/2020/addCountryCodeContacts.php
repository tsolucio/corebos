...
class addCountryCodeContacts extends cbupdaterWorker {
	...
	public function applyChange() {
		...
		if ($this->isApplied()) {
			...
			$field->table		=	'vtiger_contactaddress';
	...
	public function isApplied() {
		...
		if (!$done) {
			global $adb;
			$r = $adb->query("SHOW COLUMNS FROM `vtiger_contactaddress` LIKE '%_countrycode'");
			if ($adb->num_rows($r) > 0) {
				$done = true;
			}
		}
