...
function migration520_populateFieldForSecurity($tabid, $fieldid) {
	...
	$check_deforg_res = $adb->pquery(
		'SELECT 1 FROM vtiger_def_org_field WHERE tabid=? AND fieldid=? LIMIT 1',
		array($tabid, $fieldid)
	);
...
ExecuteQuery('CREATE TABLE vtiger_campaignrelstatus (campaignrelstatusid INTEGER, campaignrelstatus VARCHAR(200), sortorderid INT, presence INT) ENGINE=InnoDB DEFAULT CHARSET=utf8;');
...
$sql = 'update vtiger_field set block=(select blockid from vtiger_blocks where blocklabel=?) where tablename=?';
...
function VT520_webserviceMigrate() {
	...
	$operationId = vtws_addWebserviceOperation(
		$customWebserviceDetails['name'],
		$customWebserviceDetails['include'],
		$customWebserviceDetails['handler'],
		$customWebserviceDetails['type']
	);
...
$modifyevent = $adb->pquery(
	'insert into vtiger_eventhandlers(eventhandler_id, event_name, handler_path, handler_class,cond,is_active) values (?,?,?,?,?,1)',
	array($handlerId, $modifyevent, $eventPath, $handlerClass, '')
);
...
function VT520_migrateCustomview($sql, $forModule, $user, $handler) {
	...
	foreach ($it as $row) {
		...
		if (empty($moduleMetaInfo[$module])) {
			$moduleMetaInfo[$module] = new VtigerCRMObjectMeta(VtigerWebserviceObject::fromName(
				$db,
				$module
			), $user);
		}
...
ExecuteQuery('CREATE TABLE vtiger_asteriskincomingevents (
...
function VT520_manageIndexes() {
	$db = PearDatabase::getInstance();
	ExecuteQuery('ALTER TABLE vtiger_potential ADD INDEX `vt_pot_sales_stage_amount_idx` (amount, sales_stage)');
	...
	if ($count > 0) {
		...
		ExecuteQuery('ALTER TABLE vtiger_potential ADD INDEX `potential_relatedto_idx` (related_to)');
	}
	$result = $db->pquery('SELECT COUNT(1) as count FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema = ' .
	...
	ExecuteQuery('ALTER TABLE vtiger_crmentity ADD INDEX `crm_ownerid_del_setype_idx` (smownerid,deleted,setype)');
}
...
ExecuteQuery("update vtiger_version set old_version='5.1.0', current_version='5.2.0' where id=1");
