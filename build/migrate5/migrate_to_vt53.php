...
$migrationlog->debug("\n\nDB Changes from 5.2 to 5.3.0 -------- Starts \n\n");
...
foreach ($it as $row) {
	...
	$sql = "UPDATE vtiger_field,vtiger_def_org_field
		SET presence=0, vtiger_def_org_field.visible=0
...
function vt530_addFilterToListTypes() {
	...
	if ($db->num_rows($result) > 0) {
		...
		if ($db->num_rows($operationResult) <= 0) {
			$status = vtws_addWebserviceOperationParam(
				$operationId,
				$operationName,
				'Encoded',
				0
...
for ($i = 0; $i < $usersCount; ++$i) {
	...
	ExecuteQuery("UPDATE vtiger_cvadvfilter SET value=? WHERE columnname LIKE '%:assigned_user_id:%' AND value=?", array($fullName, $userName));
	ExecuteQuery("UPDATE vtiger_cvadvfilter SET value=? WHERE columnname LIKE '%:assigned_user_id1:%' AND value=?", array($fullName, $userName));
	ExecuteQuery("UPDATE vtiger_relcriteria SET value=? WHERE columnname LIKE 'vtiger_users%:user_name%' AND value=?", array($fullName, $userName));
...
function vt530_renameField($fieldInfo) {
	...
	for ($k = 0; $k < $count; $k++) {
		...
		if (!empty($matches)) {
			...
			$newColumnName =  str_replace($moduleName . '_' . $transformedFieldLabel, $moduleName . '_' . $transformedNewFieldLabel, $newColumnName);
...
$contactYahooFieldDetails = array(
	'moduleName' => 'Contacts', 'tableName' => 'vtiger_contactdetails', 'columnType' => 'VARCHAR(100)',
...
$leadYahooFieldDetails = array(
	'moduleName' => 'Leads', 'tableName' => 'vtiger_leaddetails', 'columnType' => 'VARCHAR(100)',
	'fieldName' => 'yahooid', 'fieldLabel' => 'Yahoo Id', 'columnName' => 'yahooid',
	'newFieldName' => 'secondaryemail', 'newFieldLabel' => 'Secondary Email', 'newColumnName' => 'secondaryemail'
);
vt530_renameField($leadYahooFieldDetails);

$userYahooFieldDetails = array(
	'moduleName' => 'Users', 'tableName' => 'vtiger_users', 'columnType' => 'VARCHAR(100)',
...
$sql = 'CREATE TABLE vtiger_ws_fieldinfo(
...
function vt530_updateCurrencyInfo() {
	...
	for ($i = 0; $i < $adb->num_rows($cur_result); $i++) {
		...
		$currency_exists = $adb->pquery(
			'SELECT * from vtiger_currencies WHERE currency_code=?',
			array($cur_code)
		);
...
ExecuteQuery("UPDATE vtiger_organizationdetails SET organizationname='Your Company Name' WHERE organizationname='vtiger'");
ExecuteQuery("update vtiger_version set old_version='5.2.0', current_version='5.3.0' where id=1");

$migrationlog->debug("\n\nDB Changes from 5.2 to 5.3.0  -------- Ends \n\n");
