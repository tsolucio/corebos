...
$migrationlog->debug("\n\nDB Changes from 5.3.0 to 5.4.0 -------- Starts \n\n");
...
for ($i = 0; $i < $rows; $i++) {
	...
	if (in_array($moduleName, $skipForModules)) {
		continue;
	}
...
ExecuteQuery('UPDATE vtiger_blocks SET sequence=? WHERE blocklabel=? AND tabid=?', array(2, 'LBL_FILE_INFORMATION', $documentsTabId));
ExecuteQuery('UPDATE vtiger_blocks SET sequence=? WHERE blocklabel=? AND tabid=?', array(3, 'LBL_DESCRIPTION', $documentsTabId));
...
ExecuteQuery("UPDATE com_vtiger_workflows SET defaultworkflow=1 WHERE
...
Vtiger_Cron::register('Workflow', 'cron/modules/com_vtiger_workflow/com_vtiger_workflow.service', 900, 'com_vtiger_workflow', 1, 0, 'Recommended frequency for Workflow is 15 mins');
Vtiger_Cron::register('RecurringInvoice', 'cron/modules/SalesOrder/RecurringInvoice.service', 43200, 'SalesOrder', 1, 0, 'Recommended frequency for RecurringInvoice is 12 hours');
Vtiger_Cron::register('SendReminder', 'cron/SendReminder.service', 900, 'cbCalendar', 1, 0, 'Recommended frequency for SendReminder is 15 mins');
Vtiger_Cron::register('ScheduleReports', 'cron/modules/Reports/ScheduleReports.service', 900, 'Reports', 1, 0, 'Recommended frequency for ScheduleReports is 15 mins');
Vtiger_Cron::register('MailScanner', 'cron/MailScanner.service', 900, 'Settings', 1, 0, 'Recommended frequency for MailScanner is 15 mins');

ExecuteQuery("DELETE FROM vtiger_settings_field WHERE name='LBL_ASSIGN_MODULE_OWNERS'", array());
...
for ($i = 0; $i < $adb->num_rows($result); $i++) {
	...
	if ((!(in_array($modulelabel, $present_module))) && (!(in_array($modulelabel, $skipModules)))) {
		if ($modulelabel == 'MailManager') {
			ExecuteQuery('update vtiger_tab set parent = ? where tablabel = ?', array('Tools', $modulelabel));
			ExecuteQuery('update vtiger_tab set tabsequence = -1 where tablabel = ?', array($modulelabel));
		} else {
			ExecuteQuery('update vtiger_tab set parent = ? where tablabel = ?', array($parent, $modulelabel));
		}
	}
}
...
if ($adb->num_rows($check_res) > 0) {
	ExecuteQuery($update_mapping, array(getFieldid($leadTab, 'company'), getFieldid($accountTab, 'accountname'), 0, getFieldid($potentialTab, 'potentialname')));
} else {
	ExecuteQuery($insert_mapping, array(getFieldid($leadTab, 'company'), getFieldid($accountTab, 'accountname'), null, getFieldid($potentialTab, 'potentialname'), 0));
}
...
if ($adb->num_rows($check_res) > 0) {
	ExecuteQuery($update_mapping, array(getFieldid($leadTab, 'email'), getFieldid($accountTab, 'email1'), getFieldid($contactTab, 'email'), 0));
} else {
	ExecuteQuery($insert_mapping, array(getFieldid($leadTab, 'email'), getFieldid($accountTab, 'email1'), getFieldid($contactTab, 'email'), null, 0));
}

$check_res = $adb->pquery($check_mapping, array(getFieldid($leadTab, 'firstname'), 0, getFieldid($contactTab, 'firstname'), 0));
if ($adb->num_rows($check_res) > 0) {
	ExecuteQuery($update_mapping, array(getFieldid($leadTab, 'firstname'), 0, getFieldid($contactTab, 'firstname'), 0));
} else {
	ExecuteQuery($insert_mapping, array(getFieldid($leadTab, 'firstname'), null, getFieldid($contactTab, 'firstname'), null, 0));
}

$check_res = $adb->pquery($check_mapping, array(getFieldid($leadTab, 'lastname'), 0, getFieldid($contactTab, 'lastname'), 0));
if ($adb->num_rows($check_res) > 0) {
	ExecuteQuery($update_mapping, array(getFieldid($leadTab, 'lastname'), 0, getFieldid($contactTab, 'lastname'), 0));
} else {
	ExecuteQuery($insert_mapping, array(getFieldid($leadTab, 'lastname'), null, getFieldid($contactTab, 'lastname'), null, 0));
}
...
ExecuteQuery("UPDATE vtiger_field SET columnname = 'smownerid', tablename = 'vtiger_crmentity', uitype = '53', typeofdata = 'V~M', info_type = 'BAS', quickcreate = 0, quickcreatesequence = 5
...
ExecuteQuery('UPDATE vtiger_cvcolumnlist SET columnname=? WHERE columnname=?', array($newProductHandlerColumnName, $oldProductHandlerColumnName));
ExecuteQuery('UPDATE vtiger_cvadvfilter SET columnname=? WHERE columnname=?', array($newProductHandlerColumnName, $oldProductHandlerColumnName));
...
ExecuteQuery("UPDATE vtiger_field SET columnname = 'smownerid', tablename = 'vtiger_crmentity', uitype = '53', typeofdata = 'V~M', info_type = 'BAS', quickcreate = 0, quickcreatesequence = 4
...
ExecuteQuery('UPDATE vtiger_cvcolumnlist SET columnname=? WHERE columnname=?', array($newServiceOwnerColumnName, $oldServiceOwnerColumnName));
ExecuteQuery('UPDATE vtiger_cvadvfilter SET columnname=? WHERE columnname=?', array($newServiceOwnerColumnName, $oldServiceOwnerColumnName));
...
ExecuteQuery("UPDATE vtiger_entityname SET fieldname='firstname,lastname' WHERE tabid=?", array($contactTab));
ExecuteQuery("UPDATE vtiger_entityname SET fieldname='firstname,lastname' WHERE tabid=?", array($leadTab));
ExecuteQuery("UPDATE vtiger_entityname SET fieldname='first_name,last_name' WHERE tabid=?", array($usersTab));
...
for ($i = 0; $i < $usersCount; ++$i) {
	...
	ExecuteQuery("UPDATE vtiger_cvadvfilter SET value=? WHERE columnname LIKE '%:assigned_user_id:%' AND value=?", array($fullName, $oldFullName));
	ExecuteQuery("UPDATE vtiger_cvadvfilter SET value=? WHERE columnname LIKE '%:modifiedby:%' AND value=?", array($fullName, $oldFullName));
	ExecuteQuery("UPDATE vtiger_cvadvfilter SET value=? WHERE columnname LIKE '%:assigned_user_id1:%' AND value=?", array($fullName, $oldFullName));
	ExecuteQuery("UPDATE vtiger_relcriteria SET value=? WHERE columnname LIKE 'vtiger_users%:user_name%' AND value=?", array($fullName, $oldFullName));
	ExecuteQuery("UPDATE vtiger_relcriteria SET value=? WHERE columnname LIKE '%:modifiedby:%' AND value=?", array($fullName, $oldFullName));

	ExecuteQuery("UPDATE vtiger_cvadvfilter SET comparator='c'
	...
	ExecuteQuery("UPDATE vtiger_relcriteria SET comparator='c'
...
foreach ($replaceReportColumnsList as $oldName => $newName) {
	ExecuteQuery('UPDATE vtiger_selectcolumn SET columnname=? WHERE columnname=?', array($newName, $oldName));
	ExecuteQuery('UPDATE vtiger_relcriteria SET columnname=? WHERE columnname=?', array($newName, $oldName));
	ExecuteQuery('UPDATE vtiger_reportsortcol SET columnname=? WHERE columnname=?', array($newName, $oldName));
}

// Report Charts - tables creation
ExecuteQuery('CREATE TABLE if not exists vtiger_homereportchart (stuffid int(19) PRIMARY KEY, reportid int(19), reportcharttype varchar(100))', array());
ExecuteQuery('CREATE TABLE vtiger_reportgroupbycolumn(reportid int(19),sortid int(19),sortcolname varchar(250),dategroupbycriteria varchar(250))', array());
ExecuteQuery('ALTER TABLE vtiger_reportgroupbycolumn add constraint fk_1_vtiger_reportgroupbycolumn FOREIGN KEY (reportid) REFERENCES vtiger_report(reportid) ON DELETE CASCADE', array());
ExecuteQuery("insert into vtiger_reportgroupbycolumn (reportid, sortid, sortcolname, dategroupbycriteria) select reportid, sortcolid, columnname, 'None' from vtiger_reportsortcol where columnname like '%:D'", array());

ExecuteQuery("DELETE FROM vtiger_time_zone WHERE time_zone = 'Kwajalein'", array());
ExecuteQuery("UPDATE vtiger_users SET time_zone='UTC' WHERE time_zone='Kwajalein'", array());
