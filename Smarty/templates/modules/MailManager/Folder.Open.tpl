...
<tr>
	...
	<td align="right" colspan="4" class="cblds-t-align--right cblds-p--medium">
...
<tr>
	<td colspan="5">
	<table class="mm_tableHeadBg small cblds-table-border--sep cblds-table-bordersp--small" width="100%" cellspacing="0" border="0" cellpadding="2px" >
		<tr>
			...
			<td class="moduleName cblds-t-align--right" align="right">{'LBL_Search'|@getTranslatedString}
...
<table cellpadding="0" cellspacing="0" border="0" width="100%" class="cmall mm_mailwrapper">
	...
	<tr class="mm_lvtColData {if $MAIL->isRead()}mm_normal{else}mm_bold{/if} mm_clickable"
		...
		<td width="3%" class="cblds-p--mediumlarge"><input type='checkbox' value = "{$MAIL->msgNo()}" name = 'mc_box' class='small' onclick='MailManager.toggleSelectMail(this.checked, this);'></td>
		<td width="27%" class="cblds-p--mediumlarge" onclick="MailManager.mail_open('{$FOLDER->name()}', {$MAIL->msgNo()});">{$MAIL->from(30)}</td>
		<td class="cblds-p--mediumlarge" onclick="MailManager.mail_open('{$FOLDER->name()}', {$MAIL->msgNo()});">{$MAIL->subject()}</td>
		<td class="cblds-p--mediumlarge cblds-t-align--right" width="17%" align="right" onclick="MailManager.mail_open('{$FOLDER->name()}', {$MAIL->msgNo()});">{$MAIL->date(true)}</td>
	</tr>
	{foreachelse}
</table>
