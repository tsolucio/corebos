			...
			<tr valign=top>
				...
				<form action="index.php" method="post" name="DetailView" id="form" onsubmit="VtigerJS_DialogBox.block();">
					...
					<table border=0 cellspacing=0 cellpadding=0 width=100%>
						<tr>
							<td style="padding:3px"><!-- General details --> {foreach key=header item=detail from=$BLOCKS}
							...
							{/foreach}
							{if $ACTIVITYDATA.activitytype neq 'Task'} <!-- display of fields starts -->
							<table border=0 cellspacing=0 cellpadding=5 width=100% >
								...
								<tr>
									...
									<td class="cellInfo" align="left" nowrap valign="top"> {if $ACTIVITYDATA.eventstatus eq $APP.LBL_NOT_ACCESSIBLE} <font color="red">{$ACTIVITYDATA.eventstatus}</font> {else}
								...
								<tr>
									...
									<td class="cellInfo" align="left" nowrap valign="top"> {if $ACTIVITYDATA.taskpriority eq $APP.LBL_NOT_ACCESSIBLE} <font color="red" >{$ACTIVITYDATA.taskpriority}</font> {else}
							...
							<table border=0 cellspacing=1 cellpadding=0 width=100%>
								<tr>
									...
									<table border=0 cellspacing=0 cellpadding=2 width=100%>
										<tr>
											<td class="mailSubHeader"><b>{$CMOD.LBL_EVENTSTAT}</b></td>
										</tr>
										<tr>
											<td class=small>{$ACTIVITYDATA.starthr}:{$ACTIVITYDATA.startmin}{$ACTIVITYDATA.startfmt}</td>
										</tr>
										<tr>
											<td class=small>{$ACTIVITYDATA.date_start}</td>
										</tr>
									</table></td>
									<td width=50% valign=top >
									<table border=0 cellspacing=0 cellpadding=2 width=100%>
										<tr>
											<td  class="mailSubHeader"><b>{$CMOD.LBL_EVENTEDAT}</b></td>
										</tr>
										<tr>
											<td class=small>{$ACTIVITYDATA.endhr}:{$ACTIVITYDATA.endmin}{$ACTIVITYDATA.endfmt}</td>
										</tr>
										<tr>
											<td class=small>{$ACTIVITYDATA.due_date}</td>
										</tr>
									</table></td>
								</tr>
							</table> {if $CUSTOM_FIELDS_DATA|@count > 0}
							<table border=0 cellspacing=0 cellpadding=5 width=100% >
								<tr>
									{strip} <td colspan=4 class="tableHeading"><b>{$APP.LBL_CUSTOM_INFORMATION}</b></td>{/strip}
								</tr>
								<tr>
							...
							</table> {else} <!-- detailed view of a ToDo -->
							<table border="0" cellpadding="5" cellspacing="0" width="100%">
								...
								<tr>
									...
									<table border="0" cellpadding="5" cellspacing="1" width="100%" >
										...
										<tr>
											{if $LABEL.taskstatus neq ''}
											<td class="cellInfo" align="left" valign="top"> {if $ACTIVITYDATA.taskstatus eq $APP.LBL_NOT_ACCESSIBLE} <font color="red">{$ACTIVITYDATA.taskstatus}</font> {else}
											{$ACTIVITYDATA.taskstatus}
											{/if} </td>
											{/if}
											{if $LABEL.taskpriority neq ''}
											<td class="cellInfo" align="left" valign="top"> {if $ACTIVITYDATA.taskpriority eq $APP.LBL_NOT_ACCESSIBLE} <font color="red">{$ACTIVITYDATA.taskpriority}</font> {else}
											{$ACTIVITYDATA.taskpriority}
											{/if} </td>
											{/if}
							...
							<table border="0" cellpadding="0" cellspacing="0" width="100%" align=center>
								<tr>
									...
									<table border=0 cellspacing=0 cellpadding=2 width=100% align=center>
										<tr>
											<td class="mailSubHeader" align=left ><b>{$CMOD.LBL_TIMEDATE}</b></td>
										</tr>
										<tr>
											<td class="small" >{$ACTIVITYDATA.starthr}:{$ACTIVITYDATA.startmin}{$ACTIVITYDATA.startfmt}</td>
										</tr>
										<tr>
											<td class="cellInfo" style="padding-left:0px">{$ACTIVITYDATA.date_start}</td>
										</tr>
									</table></td>
									<td width=50% valign="top">
									<table border=0 cellspacing=0 cellpadding=2 width=100% align=center>
										<tr>
											<td class="mailSubHeader"><b>{$LABEL.due_date}</b></td>
										</tr>
										<tr>
											<td class="small">{$ACTIVITYDATA.due_date}</td>
										</tr>
