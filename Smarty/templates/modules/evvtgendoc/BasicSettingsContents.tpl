...
<table border=0 cellspacing=0 cellpadding=5 width="350px" align="" class="dvtContentSpace">
	...
	{assign var='i18nEnable' value='LBL_ENABLE'|@getTranslatedString:'Settings'}
	{assign var='i18nDisable' value='LBL_DISABLE'|@getTranslatedString:'Settings'}
	...
	<tr onmouseover="this.className='prvPrfHoverOn'" onmouseout="this.className='prvPrfHoverOff'">
		...
		{if $module.active eq '1'}
			<a href="javascript:void(0);" onclick="toggleModule_mod('{$tabid}', 'module_disable');">
			 <img src="{'enabled.gif'|@vtiger_imageurl:$THEME}" border="0" align="absmiddle" alt="{$i18nDisable} {$module.name}" title="{$i18nDisable} {$module.name}">
			</a>
		{else}
			<a href="javascript:void(0);" onclick="toggleModule_mod('{$tabid}', 'module_enable');">
			<img src="{'disabled.gif'|@vtiger_imageurl:$THEME}" border="0" align="absmiddle" alt="{$i18nEnable} {$module.name}" title="{$i18nEnable} {$module.name}">
			</a>
		{/if}
		</td>
	</tr>
