	...
	<div class="slds-modal__content slds-app-launcher__content slds-p-around_medium">
		{if $ISADMIN}
			<form role="form" style="margin:0 100px;" id="cache_form">
				...
				<input type="hidden" name="_op" value="setconfigredis">
				<div class="slds-form-element">
					<label class="slds-checkbox_toggle slds-grid">
						<span class="slds-form-element__label slds-m-bottom_none">{'_active'|@getTranslatedString:$MODULE}</span>
						<input type="checkbox" name="redis_active" aria-describedby="toggle-desc" {if $isActive}checked{/if} />
				...
				<div class="slds-m-top_large">
					<button type="button" class="slds-button slds-button_brand" onclick="Utilities.cache_form_submit_validation(this);">{'LBL_SAVE_BUTTON_LABEL'|@getTranslatedString:$MODULE}</button>
				</div>
			</form>
		{/if}
	</div>
