...
<tr>
	...
	<div align=center>
		...
		<input type="hidden" name="returnaction" value="{$RETURN_ACTION}">
			<tr>
				<td width=50 rowspan=2 valign=top class="cblds-p--none"><img src="{'ico-groups.gif'|@vtiger_imageurl:$THEME}" alt="{$CMOD.LBL_GROUPS}" title="{$CMOD.LBL_GROUPS}" width="48" height="48" border=0 ></td>
				{if $MODE eq 'edit'}
			...
			<tr>
				{if $MODE eq 'edit'}
				<td valign=top class="small cblds-p-v--none">{$MOD.LBL_EDIT} {$CMOD.LBL_PROPERTIES} &quot;{$GROUPNAME}&quot; {$CMOD.LBL_GROUP}</td>
				{else}
				<td valign=top class="small cblds-p-v--none">{$CMOD.LBL_NEW_GROUP}</td>
				{/if}
			</tr>
		</table>
