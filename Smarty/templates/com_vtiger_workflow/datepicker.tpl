...
<li>
	...
	{foreach $multipleschtime_12h as $schtime}
		...
		<span class="slds-pill slds-pill_link" id="{preg_replace('/\s+|\:/','-',$schtime)}">
			...
			<a class="slds-button slds-button_icon slds-button_icon slds-pill__remove" title="{'LBL_REMOVE'|@getTranslatedString:'PickList'}" onclick="removeSchTime('{$schtime}')">
				<svg class="slds-button__icon" aria-hidden="true">
					<use xlink:href="include/LD/assets/icons/utility-sprite/svg/symbols.svg#close"></use>
				</svg>
				<span class="slds-assistive-text">{'LBL_REMOVE'|@getTranslatedString:'PickList'}</span>
			</a>
		</span>
		{/if}
	{/foreach}
			...
			<div class="slds-combobox slds-dropdown-trigger slds-dropdown-trigger_click slds-timepicker" id="chtimemultipletimers">
				<div class="slds-combobox__form-element slds-input-has-icon slds-input-has-icon_right" role="none">
					<input type="text" class="slds-input slds-combobox__input slds-has-focus" id=""
						...
						placeholder="{'LBL_SELECT_TIME'|@getTranslatedString:'Reports'}" value="" onclick="triggerTimeInput()">
