			...
			{if isset($show)}
				...
				<div class="slds-grid slds-gutters slds-m-around_xxx-small">
					<div class="slds-col">
						...
						{/if}
						<button class="slds-button slds-button_success" type="button" id='save_submit' style="display:none;" {if $workflow->executionConditionAsLabel() eq 'MANUAL'} onclick="confirm_changing()"{/if}>
							<svg class="slds-button__icon slds-button__icon_left" aria-hidden="true">
								<use xlink:href="include/LD/assets/icons/utility-sprite/svg/symbols.svg#save"></use>
							</svg>
							{$APP.LBL_SAVE_LABEL}
						...
						<script type="text/javascript">
							function confirm_changing() {
								...
								if (comp_val != comp_val2) {
									window.alert('You are changing the title of a manual trigger point workflow');
								}
							}
						</script>
