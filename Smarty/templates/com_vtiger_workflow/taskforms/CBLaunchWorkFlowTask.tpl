        ...
        <table class="slds-table slds-table_cell-buffer" id="questionDiv" style="width:100%;">
            <tr>
                <td>
                    ...
                    <span>{'LBL_EXECUTE_THIS_WORKFLOW'|@getTranslatedString:'LBL_EXECUTE_THIS_WORKFLOW'}</span>
                    <input id="workflowid_display" name="workflowid_display" readonly type="text" style="border:1px solid #bababa;" onclick='return vtlib_open_popup_window("new_task_form", "workflowid", "com_vtiger_workflow", "");' {if !empty($task->workflowid_display )} value = "{$task->workflowid_display}" {else} value ="" {/if}>&nbsp;
            ...
            <tr>
                <td>
                    ...
                    <input type="text" class="slds-input" style="width:50%;" name="crmids_list" id="crmids_list" {if !empty($task->crmids_list )} value = "{$task->crmids_list}" {else} value ="" {/if}>
                </td>
            </tr><br />
            <tr>
                <td>
                    ...
                    <select class="slds-select" name="relModlist" id="relModlist" style="width:15%;" onchange="filterWorkFlowBasedOnRelatedModule()">
            ...
            <tr>
                <td>
                    ...
                    <label class="slds-radio__label" for="record_filter_opt_1">
                        <span class="slds-radio_faux"></span>
                        <span class="slds-form-element__label">{'LBL_ON_THESE'|@getTranslatedString}</span>
                    </label>
                    ...
                    {* <input type='hidden' class='small' name="workflow_type" id="workflow_type" value="com_vtiger_workflow"> *}
                    <span>{'LBL_WITH_THIS_CONDITION'|@getTranslatedString:'LBL_WITH_THIS_CONDITION'}</span>
                    <input id="recordsetmapid_display" name="recordsetmapid_display" readonly type="text" style="border:1px solid #bababa;" onclick="return window.open('index.php?module=cbMap&action=Popup&html=Popup_picker&form=new_task&forfield=recordsetmapid&srcmodule=GlobalVariable'+SpecialSearchRecordSetMapping, 'vtlibui10wf', cbPopupWindowSettings);" {if !empty($task->recordsetmapid_display )} value = "{$task->recordsetmapid_display}" {else} value ="" {/if}>&nbsp;
