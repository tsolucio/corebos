...
{foreach item=elements from=$row}
	...
	<td colspan="3" class="mailSubHeader" style="padding: 5px;" align="center">
		<input type="checkbox" id="individual_emails" name="individual_emails" onclick="toggleCheckbox('individual_emails')" value="{$SEND_INDIVIDUAL_EMAILS}" {if $SEND_INDIVIDUAL_EMAILS eq 1}checked{/if}/>{$MOD.LBL_SEND_INDIVIDUAL_EMAILS}
