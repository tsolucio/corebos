...
<!-- Code added by SAKTI on 17th Jun, 2008 -->

<table width="100%" cellpadding="2" cellspacing="0" border="0">
	<tr>
		<td>&nbsp;</td>
		<td>
			<table cellpadding="0" cellspacing="5" border="0"></table>
						...
						<tr>
							<td style="padding:3px">
													...
													<div id="addEventRelatedtoUI" style="display:none;width:100%">
														<table width="100%" cellpadding="5" cellspacing="0" border="0">
															...
															<tr>
																...
																{foreach item=contactname key=cntid from=$CONTACTS}{$contactname}<br>{/foreach}
															</tr>
														</table>
													</div>
											</td>
								...
								{/if}
										...
										{if $CUSTOM_FIELDS_DATA|@count > 0}
											<table border=0 cellspacing=0 cellpadding=5 width=100% >
												<tr>{strip}<td colspan=4 class="tableHeading"><b>{$APP.LBL_CUSTOM_INFORMATION}</b></td>{/strip}</tr>
												<tr>
													{foreach key=index item=custom_field from=$CUSTOM_FIELDS_DATA}
														{assign var=keyid value=$custom_field.2}
														{assign var=keyval value=$custom_field.1}
