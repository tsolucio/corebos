...
<table class="slds-table slds-table--bordered slds-no-row-hover slds-table--fixed-layout ld-font">
	...
	{foreach key=index item=_HEADER_FIELD from=$RELATEDLISTDATA.header}
		<th class="slds-text-title--caps {if $RELATED_MODULE eq 'Contacts'} fix-width {/if}" scope="col">
			<span class="slds-truncate slds-text-link--reset" style="padding: .5rem 0;">{$_HEADER_FIELD}</span>
		</th>
	{/foreach}
	</tr>
	...
	<tr class="rel_mod_data_campaigns">
		<td id="{$MODULE}_{$RELATED_MODULE}_linkForSelectAll" class="linkForSelectAll" style="display:none;" colspan=8>
	...
	<tr class="slds-hint-parent slds-line-height--reset" id="row_{$_RECORD_ID}">
		...
		<th scope="row">
			<div class="slds-truncate" onmouseover="vtlib_listview.trigger('cell.onmouseover', this)" onmouseout="vtlib_listview.trigger('cell.onmouseout', this)">{$_RECORD_DATA}</div>
		</th>
		{/foreach}
	</tr>
	{foreachelse}
	<tr class="slds-line-height--reset">
		<th scope="row"><i>{$APP.LBL_NONE_INCLUDED}</i></th>
	</tr>
	{/foreach}
</table>
