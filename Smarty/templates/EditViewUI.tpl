...
alt="{'LBL_CLEAR'|@getTranslatedString}" title="{'LBL_CLEAR'|@getTranslatedString}" onClick="this.form.{$fldname}.value=''; this.form.{$fldname}_display.value=''; return false;" align="absmiddle" style='cursor:hand;cursor:pointer'>&nbsp;
		...
		{elseif $uitype eq 1}
			...
			<td id="td_val_{$fldname}" width=30% align=left class="dvtCellInfo">
				<div style="position: relative;">
					{if isset($FIELDS_NAME[{$fldname}])} {$val = $MAP_VALUES[{$fldname}]} {else} {$val = ''} {/if}
					<input 
						type="text" 
						tabindex="{$vt_tab}"
						name="{$fldname}"
						...
						autocomplete="off"
						role="textbox"
						{if $val neq ''}onfocus="Uitype1FieldsAutocomplete('{$fldname}', '{$val}');"{/if}
						style="margin-top: 3px; margin-bottom: 3px; width: 90%;"
						data-autocomp="">
						<div id="listbox-unique-id-{$fldname}" role="listbox" class=""></div>
				</div>
			</td>
		{elseif $uitype eq 9}
