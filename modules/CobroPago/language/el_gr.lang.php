<?php
/*+**********************************************************************************
 * The contents of this file are subject to the vtiger CRM Public License Version 1.0
 * ("License"); You may not use this file except in compliance with the License
 * The Original Code is:  vtiger CRM Open Source
 * The Initial Developer of the Original Code is vtiger.
 * Portions created by vtiger are Copyright (C) vtiger.
 * All Rights Reserved.
 ************************************************************************************/

$mod_strings = array(
'LBL_MODULE_NAME'=>'Έσοδα/Έξοδα',
'SINGLE_CobroPago'=>'Έσοδα/Έξοδα',
'LBL_MODULE_TITLE'=>'Έσοδα/Έξοδα: Αρχική',
'LBL_SEARCH_FORM_TITLE'=>'Αναζήτηση Πληρωμών',
'LBL_LIST_FORM_TITLE'=>'Λίστα Πληρωμών',
'LBL_COBROPAGO_NAME'=>'Ονομασία Πληρωμής:',
'LBL_COBROPAGO'=>'Πληρωμή:',
'LBL_NAME'=>'Ονομασία Πληρωμής',
'LBL_INVITEE'=>'Επαφές',
'LBL_LIST_COBROPAGO_NAME'=>'Έσοδα/Έξοδα',
'LBL_LIST_ACCOUNT_NAME'=>'Ονομασία Εταιρείας',
'LBL_PRODUCT_NAME'=>'Ονομασία Προϊόντος',
'LBL_LIST_AMOUNT'=>'Ποσότητα',

'LBL_ACCOUNT_NAME'=>'Ονομασία Εταιρείας:',
'LBL_AMOUNT'=>'Ποσότητα:',
'LBL_TYPE'=>'Τύπος:',
'LBL_PROBABILITY'=>'Πιθανότητα (%):',
'LBL_DESCRIPTION'=>'Περιγραφή:',
'LBL_COBROPAGO_INFORMATION'=>'Πληροφορίες Πληρωμής:',
'LBL_DESCRIPTION_INFORMATION'=>'Πληροφορίες Περιγραφής:',
'LBL_CUSTOM_INFORMATION'=>'Ειδικές Πληροφορίες',

'LBL_NEW_FORM_TITLE'=>'Νέα Πληρωμή',

'ERR_DELETE_RECORD'=>'Πρέπει να δωθεί αριθμός εγγραφής για να διαγραφεί αυτή η πληρωμή.',
'LBL_TOP_PAYMENTS'=>'Οι Σημαντικότερες Ανοιχτές Πληρωμές μου',

'NTC_NONE_SCHEDULED'=>'Καμία Προγραμματισμένη.',

'LBL_SELECT_COBROPAGO'=>'Επιλογή Πληρωμής',
'LBL_GENERAL_INFORMATION'=>'Γενικές Πληροφορίες',

//for v4 release added
'LBL_NEW_CobroPago'=>'Νέα Πληρωμή',
'LBL_CobroPago_TITLE'=>'Έσοδα/Έξοδα',

// Added for 4GA
'LBL_TOOL_FORM_TITLE'=>'Εργαλεία Πληρωμών',
//Added for 4GA
'CobroPago Name'=>'Ονομασία Πληρωμής',
'Reference'=>'Ονομασία Πληρωμής',
'Register'=>'Ημερομηνία Καταγραφής',
'Paid'=>'Εξοφλημένη',
'Credit'=>'Πίστωση',
'Amount'=>'Ποσό',
'Benefit'=>'Κέρδος',
'Cost'=>'Κόστος',
'Account Name'=>'Ονομασία Εταιρείας',
'Parent'=>'Εταιρεία/Επαφή',
'Currency'=>'Νόμισμα',
'RelatedTo'=>'Σχετική Οντότητα',
'PaymentMode'=>'Τρόπος Πληρωμής',
'Category'=>'Κατηγορία Πληρωμής',
'CobroPago Type'=>'Τύπος Πληρωμής',
'Type'=>'Τύπος',
'Probability'=>'Πιθανότητα (%)',
'CobroPago'=>'Έσοδα/Έξοδα',
'CobroPago ID'=>'Κωδικός Πληρωμής',
'cobropagoid'=>'Κωδικός Πληρωμής',
'DueDate'=>'Προθεσμία',
'Comercial'=>'Σχετικός χρήστης',
'Recibo'=>'Απόδειξη',
'Historico de Cobros y Pagos'=>'Ιστορικό Εσόδων/Εξόδων',
'Payment Paid'=>'Η πληρωμή έχει σημειωθεί ως ΕΞΟΦΛΗΜΕΝΗ από ',
'PaidOn'=>' σε ',
'RecieptOpen'=>'Η απόδειξη εκτυπώθηκε από ',
'CyP No'=>'Payment No',
'PaymentDate'=>'Payment Date',

//Added for Existing Picklist Strings

'Cash'=>'Ρευστό',
'Check'=>'Επιταγή',
'Credit card'=>'Πιστωτική κάρτα',
'Infrastructure'=>'Υποδομή',
'Stock'=>'Απόθεμα',
'Sale'=>'Πώληση',

'Other'=>'Άλλο',

'--None--'=>'--Κανένα--',
'LBL_CHANGE'=>'Χρήστης Πωλήσεων',

'LBL_CARD_INFORMATION' => 'Card Information',
'CARD_ACCOUNT' => 'Card Number:',
'CARD_CVV' => 'CVV2:',
'CARD_EXPIRY_DATE' => 'Expiry Date (MMYY):',
'CARD_SUBMIT' => 'Send',
'ACH_ACCOUNT_TYPE' => 'Account Type:',
'ACH_ACCOUNT_NUMBER' => 'Account Number:',
'ACH_ROUTING_NUMBER' => 'Routing Number:',
'ACH_SUBMIT' => 'Send',
'Receive BluePay Payment'=>'Receive BluePay Payment',

'ERR_ALREADY_PAID' => "This payment has been already paid.",
'ERR_NO_ACCOUNT' => "There's no related account.",
'ERR_NO_TOKEN' => "No token for this account.",

'Pay' => 'Pay',

'PreventEdit' => 'Prevent edit/delete if paid',
);
?>
