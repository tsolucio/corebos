...
$uploaded = move_uploaded_file($file_tmp, 'build/'.$upload_file);
if ($uploaded) {
	$xmlreader = new SimpleXMLElement('build/'.$upload_file, 0, true);
	if (($category == 'GROUP' || $category == 'ROLE' || $category == 'PROFILE')) {
		if ($category == 'ROLE') {
