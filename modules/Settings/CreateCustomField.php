...
for ($i=0; $i<count($cftextcombo); $i++) {
	...
	$combo_output.= '<a href="javascript:void(0);" onClick="makeFieldSelected(this,'.$i.','.$blockid.');" id="field'.$i
		.'" style="text-decoration:none;background-image:url('.$cfimagecombo[$i].');" class="customMnu" '.$disable_str.'>'.$cftextcombo[$i].'</a>';
}
...
if ($mode == 'edit') {
	...
	$output .= '<td width="60%" align="left" class="layerPopupHeading">'.$mod_strings['LBL_ADD_FIELD'].'</td>';
			...
			$output .= '<td width="40%" align="right"><a href="javascript:fninvsh(\'customfield\');"><img src="'. vtiger_imageurl('close.gif', $theme)
					...
					<table width="100%" border="0" cellpadding="5" cellspacing="0">
						<tr>
							...
							<td align="left" width="70%">
								<input name="fldLabel_'.$blockid.'" id ="fldLabel_'.$blockid.'" value="'.$customfield_fieldlabel.'" type="text" class="txtBox"></td>
						</tr>';
if ($mode != 'edit') {
	$output .= '<tr id="lengthdetails_'.$blockid.'">
							<td class="dataLabel" nowrap="nowrap" align="right"><b>'.$mod_strings['LBL_LENGTH'].'</b></td>
							<td align="left">
								<input type="text" name="fldLength_'.$blockid.'" id= ="fldLength_'.$blockid.'" value="'.$fieldlength.'" '.$readonly.' class="txtBox"></td>
						</tr>
						<tr id="decimaldetails_'.$blockid.'" style="display:none;">
							...
							<td align="left">
								<input type="text" name="fldDecimal_'.$blockid.'" id= "fldDecimal_'.$blockid.'" value="'.$decimalvalue.'" '.$readonly.' class="txtBox">
							</td>
						</tr>
						<tr id="picklistdetails_'.$blockid.'" style="display:none;">
							...
							<td align="left" valign="top">
								<textarea name="fldPickList_'.$blockid.'"  id = ="fldPickList_'.$blockid.'" rows="10" class="txtBox" '.$readonly.'>'.$fldVal.'</textarea>
							</td>
						</tr>';
}
				$output .= '
