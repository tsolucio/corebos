...
if (isset($_REQUEST['flag']) && $_REQUEST['flag'] != '') {
	...
	switch ($flag) {
		...
		case 2:
			$smarty->assign(
				'ERRORFLAG',
				"<font color='red'><B>".$mod_strings['Error_Message']."<ul><li><font color='red'>".$mod_strings['Invalid_file']."</font><li><font color='red'>"
					.$mod_strings['File_has_no_data']."</font></ul></B></font>"
			);
			break;
		case 3:
			$smarty->assign(
				'ERRORFLAG',
				"<B><font color='red'>".$mod_strings['Sorry'].','.$mod_strings['uploaded_file_exceeds_maximum_limit'].'.'.$mod_strings['try_file_smaller'].'</font></B>'
			);
			break;
		case 4:
