...
global $mod_strings, $app_strings, $current_language, $adb, $log, $theme;
...
if ($mode == 'addnew') {
	...
	$sql_maxsort ="select max(sortid)+1 as sortid
	...
	$sql_insert = "insert into vtiger_".$tableName." values(?,?,?,?)";
	foreach ($newPicklist as $key => $val) {
		$val = urldecode($val);
		$picklistvalue_id = getUniquePicklistID();
		$picklist_id = $adb->getUniqueID('vtiger_'.$tableName);
		$adb->pquery($sql_insert, array($picklist_id, $val, 1, $picklistvalue_id));
		foreach ($roleIds as $roleid) {
			$msort = $adb->pquery($sql_maxsort, array($roleid, $picklistid));
			$sortid = $adb->query_result($msort, 0, 'sortid');
			if ($sortid == '') {
				$sortid = 0;
			}
			$adb->pquery('insert into vtiger_role2picklist values(?,?,?,?)', array($roleid, $picklistvalue_id, $picklistid, $sortid));
		}
	}
	echo ':#:SUCCESS';
	exit;
...
if ($mode == 'replace') {
	...
	foreach ($unwantedPicklist as $key => $val) {
		$result = $adb->pquery('select tablename,columnname,uitype from vtiger_field where fieldname=?', array($fieldName));
		...
		if ($num > 0) {
			for ($n=0; $n<$num; $n++) {
				...
				if ($mode_type == 'delete') {
					$adb->pquery("update $table_name set $column_name=? where $column_name=?", array($replaceWith, $val));
					$adb->pquery(
						"delete from vtiger_role2picklist where picklistvalueid in (select picklist_valueid from vtiger_$fieldName where $fieldName=?)",
						array($val)
					);
					$adb->pquery("delete from vtiger_$fieldName where $fieldName=?", array($val));
				}
				if ($mode_type == 'edit') {
					$cnt_res = $adb->pquery("select $fieldName from vtiger_$fieldName where $fieldName = ?", array($replaceWith));
					if ($adb->num_rows($cnt_res) <= 0) {  // Avoid inserting duplicate entries for the new picklist value
						...
						$adb->pquery("insert into vtiger_$fieldName values(?,?,?,?)", array($picklist_id, $replaceWith, 1, $picklistvalue_id));

						$old_res = $adb->pquery("select * from vtiger_$fieldName where $fieldName=?", array($val));
						$old_picklistvalue_id = $adb->query_result($old_res, 0, 'picklist_valueid');

						foreach ($roleIds as $roleid) {
							$adb->pquery(
								'update vtiger_role2picklist set picklistvalueid=? where picklistvalueid=? and picklistid=? and roleid=?',
								array($picklistvalue_id, $old_picklistvalue_id, $picklistid, $roleid)
							);
						}
					}
				}
...
if (isset($fieldName)) {
	...
	$res_ult = $adb->query("select $fieldName from vtiger_$fieldName where presence=0");
	...
	if ($mode == 'transfer') {
		...
		$res=$adb->query("select $fieldName from vtiger_$fieldName where $fieldName not in ($pick_str)");
		...
		$output="<table border=0 cellspacing=0 cellpadding=5 width=100%>
			<tr><td colspan=2 align='center'><strong> ".$mod_strings['LBL_PICKLIST_TRANSFER']."\"".$fieldLabel."\"</strong></td><td align='right'><img src='"
			. vtiger_imageurl('close.gif', $theme) . "' align='middle' border='0' onclick=hide('transferdiv');></td></tr>
			<tr><td></td><td></td><td></td></tr>
			<tr><td class='small' align='right'><b>".$mod_strings['LBL_REPLACE_VALUE_WITH'].":</b></td>"
			."<td align='left'><select style='width:180px;font-size:normal;' class='small detailedViewTextBox' id='replacePick'>$option</select></td>
			<td></td></tr>
			<tr><td colspan=3 align='center'><input type='button' name='replaceText' value='".$app_strings['LBL_REPLACE_LABEL']
			."' onClick=pickReplace('".$moduleName."','".$fieldName."','delete'); class='crmButton small save'>&nbsp;<input type='button' value='"
			.$app_strings['LBL_CANCEL_BUTTON_LABEL']."' name='cancel' class='crmButton small cancel' onclick=\"hide('transferdiv');\"></td></tr>
			</table>";
