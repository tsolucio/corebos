...
class HTML_Safe {
	...
	public function openHandler(&$parser, $name, $attrs) {
		...
		if ((isset($this -> counter['table'])) && ($this -> counter['table'] <= 0) && (in_array($name, $this -> tableTags))) {
			return true;
		}
	...
	public function closeHandler(&$parser, $name) {
		...
		if (isset($this -> dcCounter[$name]) && ($this -> dcCounter[$name] > 0) && (in_array($name, $this -> deleteTagsContent))) {
