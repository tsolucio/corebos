...
class Contacts extends CRMEntity {
	...
	// pag
	var $related_tables = Array('vtiger_contactscf' => Array ('contactid', 'vtiger_contactdetails', 'contactid'));
	//
...
//////////////////////////////////////////////////////////////////////////
	...
	*/
	function __getParentContacts($id, &$parent_contacts, &$encountered_contacts) {
		global $log, $adb;
        $log->debug("Entering __getParentContacts(".$id.",".$parent_contacts.") method ...");
		...
		foreach($this->list_fields_name as $fieldname=>$columnname) {	
			...
			if ($columnname == 'account_id') {
				...
				$parent_contact_info[$columnname] = $accountname;	
			} else {
	...
	*/
	function __getChildContacts($id, &$child_contacts, $depth) {
		global $log, $adb;
        $log->debug("Entering __getChildContacts(".$id.",".$child_contacts.",".$depth.") method ...");
			...
			for($i=0;$i<$num_rows;$i++) {
				...
				foreach($this->list_fields_name as $fieldname=>$columnname) {
					if ($columnname == 'account_id') {
						...
						$accountname = getAccountName($accountid);
						$child_contact_info[$columnname] = $accountname;
					} else {
						if ($columnname == 'assigned_user_id') {
