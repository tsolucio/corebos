...
for ($x=0; $x<$y; $x++) {
	...
	if ($columnname == 'smownerid') {
		if ($modulename == 'Accounts') {
			$querycolumns[$x] = "case when (vtiger_users.user_name not like '') then ename else vtiger_groups.groupname end as userjoinname,"
		...
		if ($modulename == 'Contacts') {
			$querycolumns[$x] = "case when (usersContacts.user_name not like '') then ename else groupsContacts.groupname end as userjoincname";
		}
	}
