<?php
/**
* OpenDocument_ReferenceMark class
*
...
/**
* OpenDocument_ReferenceMark element
*
* @category   File Formats
* @package    OpenDocument
...
*/
class OpenDocument_ReferenceMark extends OpenDocument_StyledElement {
	/**
	 * Node namespace
	 */
	...
	public function __construct(DOMNode $node, OpenDocument $document, $content) {
		...
		$name = $node->getAttributeNS(OpenDocument::NS_TEXT, 'name');
		if (!empty($name)) {
			$this->node->setAttributeNS(OpenDocument::NS_TEXT, 'name', $name);
			$this->name = $name;
		}
	...
	public static function instance($object, $content = '') {
		...
		$element = new OpenDocument_ReferenceMark($node->ownerDocument->createElementNS(self::nodeNS, self::nodeName), $document, $content);
