...
function get_new_record_form () {
	...
	for ($i=0;$i<count($the_boxes);$i++) {
		$the_form .= "<a href='index.php?module=Webmails&action=index&mailbox=".preg_replace(array("/\{.*?\}/i"),array(""),$the_boxes[$i]).
			"' id='".$the_boxes[$i]."'>".preg_replace(array("/\{.*?\}/i"),array(""),$the_boxes[$i])."</a><br>";
	}
	$the_form .= "</td></tr></tbody></table>\n";
