 ...
 *  @param string 'listview'|'hourview'|empty. if 'listview' means get Events ListView, if 'hourview' means gets Events HourView. if empty means get Todos ListView
 *  @param string 'todo' or 'event'. if 'todo' means Todos View else Events View
 */
function calendar_layout(&$param_arr, $viewBox = '', $subtab = '') {
	global $mod_strings,$cal_log;
	$cal_log->debug('> calendar_layout');
	$cal_header = array ();
	...
	if (isset($param_arr['size']) && $param_arr['size'] == 'small') {
		...
		if ($subtab == 'event') {
			...
			$todo_anchor = "<a href='index.php?module=cbCalendar&action=index&view=".$cal_header['view'].$cal_header['calendar']->date_time->get_date_str()
				."&viewOption=".$viewBox."&subtab=todo&$onlyForUserParam'>".$todolabel."</a>";
		} elseif ($subtab == 'todo') {
			$eventtab_class = 'dvtUnSelectedCell';
			$todotab_class = 'dvtSelectedCell';
			// Added User Based CustomView for Calendar module
			$event_anchor = "<a href='index.php?module=cbCalendar&action=index&view=".$cal_header['view'].$cal_header['calendar']->date_time->get_date_str()
				."&viewOption=".$viewBox."&subtab=event&$onlyForUserParam'>".$eventlabel."</a>";
...
function get_mini_calendar(&$cal) {
	global $cal_log, $mod_strings, $theme;
	$cal_log->debug('> get_mini_calendar');
	$count = 0;
	...
	$minical .= "<a style='text-decoration: none;' href='index.php?module=Calendar&action=index&view=".$cal['view'].$cal['calendar']->date_time->get_date_str()."'><b>";
	$minical .= display_date($cal['view'], $cal['calendar']->date_time)."</b></a>&nbsp;".get_next_cal($cal)."</td><td class='calHdr' align='right'>";
	$minical .= "<a href='javascript:ghide(\"miniCal\");'><img src='".vtiger_imageurl('close.gif', $theme)."' align='right' border='0'></a>
		</td></tr>";
	...
	for ($i = 0; $i < $rows; $i++) {
		...
		for ($j = 0; $j < 7; $j ++) {
			...
			if ($j == 0) {
				$minical .= "<td style='text-align:center' ><a href='index.php?module=cbCalendar&action=index&view=week".$cal['slice']->start_time->get_date_str()
 ...
 * @param string $viewBox 'listview'|'hourview'|empty. if 'listview' means Events ListView.if 'hourview' means Events HourView. if empty means get Todos ListView
 * @param string $subtab  'todo' or 'event'. if 'todo' means Todos View else Events View
 */
function get_cal_header_tab(&$header, $viewBox, $subtab) {
	global $mod_strings,$cal_log;
	$cal_log->debug('> get_cal_header_tab');
	$tabhtml = '';
	$count = 1;
	...
	foreach ($links as $link) {
		if ($header['view'] == $link) {
			...
			$class = 'calUnSel';
			$anchor = "<a href='index.php?module=cbCalendar&action=index&view=".$link.$header['calendar']->date_time->get_date_str()."&viewOption=".$viewBox
				."&subtab=".$subtab."'>".$mod_strings["LBL_".$header['calendar']->getCalendarView($link)]."</a>";
		}
	...
	$tabhtml .= "<td width='30%'>
		...
		<tr>
			<td>".get_previous_cal($header, $viewBox, $subtab).'</td>';
	...
	$tabhtml .= "<td width='2%'><img onClick='fnvshobj(this,\"miniCal\"); getMiniCal(\"view=".$header['calendar']->view.$header['calendar']->date_time->get_date_str();
	$tabhtml .= "&viewOption=".$viewBox."&subtab=".$subtab."\");' src='".$header['IMAGE_PATH']."btnL3Calendar.gif' alt='".$mod_strings['LBL_OPENCAL']."...' title='";
	$tabhtml .= $mod_strings['LBL_OPENCAL']."...' align='absmiddle' border='0'></td><td width=20% >";
	$tabhtml .= "<img onClick='fnvshobj(this,\"calSettings\"); getCalSettings(\"view=".$header['calendar']->view.$header['calendar']->date_time->get_date_str();
	$tabhtml .= "&viewOption=".$viewBox."&subtab=".$subtab."\");' src='".$header['IMAGE_PATH']."tbarSettings.gif' alt='".$mod_strings['LBL_SETTINGS'];
	$tabhtml .= "' title='".$mod_strings['LBL_SETTINGS']."' align='absmiddle' border='0'></td><td class='calHdr calTopRight componentName'>";
	$tabhtml .= $app_strings['Calendar']."</td></tr>";
	echo $tabhtml;
	$cal_log->debug('> get_cal_header_tab');
}
...
function get_cal_header_data(& $cal_arr, $viewBox, $subtab) {
	...
	$cal_log->debug('> get_cal_header_data');
	...
	$headerdata ="<div style='display: block;' id='mnuTab'>
	...
	if (isPermitted('cbCalendar', "CreateView") == "yes") {
		$headerdata .="<tr>
			<td>
			<table><tr><td class='calAddButton' style='cursor:pointer;height:30px' align='center' width='15%' onMouseOver='fnAddEvent(this,\"addEventDropDown\",\""
				.$temp_date.'","'.$temp_date.'","'.$time_arr['starthour'].'","'.$time_arr['startmin'].'","'.$time_arr['startfmt'].'","'.$time_arr['endhour'].'","'
				.$time_arr['endmin'].'","'.$time_arr['endfmt'].'","'.$viewBox.'","'.$subtab.'","'.$eventlist."\");'>".$mod_strings['LBL_ADD']
				."<img src='".vtiger_imageurl('menuDnArrow.gif', $theme)."' style='padding-left: 5px;' border='0'></td></tr></table> </td>";
	} else {
		$headerdata .="<tr><td>&nbsp;</td>";
	}
	//USER SELECT CUSTOMIZATION
...
function getEventViewOption(&$cal, $viewBox) {
	...
	$cal_log->debug('> getEventViewOption');
 ...
 * @param array  collection of objects and strings
 * @param string listview|hourview|empty. listview means previous link in Events ListView. hourview means previous link in Events HourView. empty means previous link in Todos ListView
 * @param string 'todo' or 'event' or may be empty. if 'todo' means Todos View. if 'event' means Events View. if empty means small calendar view.
 * @return string  html tags in string format
 */
function get_previous_cal(& $cal, $viewBox = '', $subtab = '') {
	global $cal_log, $theme;
	$cal_log->debug('> get_previous_cal');
	if (isset($cal['size']) && $cal['size'] == 'small') {
		$link = "<a href='javascript:getMiniCal(\"view=".$cal['calendar']->view.$cal['calendar']->get_datechange_info('prev')."\")'><img src= '".
			vtiger_imageurl('small_left.gif', $theme)."' border='0' align='absmiddle' /></a>";
	} else {
		$link = "<a href='index.php?action=index&module=cbCalendar&view=".$cal['calendar']->view.$cal['calendar']->get_datechange_info('prev')."&viewOption=".$viewBox
			."&subtab=".$subtab."&onlyforuser=".calendarview_getSelectedUserId()."' onclick='VtigerJS_DialogBox.block();return true;'><img src='"
			.$cal['IMAGE_PATH']."cal_prev_nav.gif' border='0' align='absmiddle' /></a>";
	}
	$cal_log->debug('< get_previous_cal');
	return $link;
}
 ...
 * @param array  collection of objects and strings
 * @param string listview|hourview|empty. listview means next link in Events ListView. hourview means next link in Events HourView. empty means next link in Todos ListView
 * @param string 'todo' or 'event' or may be empty. if 'todo' means Todos View. if 'event' means Events View. if empty means small calendar view.
 * @return string html tags in string format
 */
function get_next_cal(& $cal, $viewBox = '', $subtab = '') {
	global $cal_log, $theme;
	$cal_log->debug('> get_next_cal');
	if (isset($cal['size']) && $cal['size'] == 'small') {
		$link = "<a href='javascript:getMiniCal(\"view=".$cal['calendar']->view.$cal['calendar']->get_datechange_info('next')."\")' ><img src='"
			.vtiger_imageurl('small_right.gif', $theme)."' border='0' align='absmiddle' /></a>";
	} else {
		$link = "<a href='index.php?action=index&module=cbCalendar&view=".$cal['calendar']->view.$cal['calendar']->get_datechange_info('next')."&viewOption=".$viewBox
			."&subtab=".$subtab."&onlyforuser=".calendarview_getSelectedUserId()."' onclick='VtigerJS_DialogBox.block();return true;'><img src='"
			.$cal['IMAGE_PATH']."cal_next_nav.gif' border='0' align='absmiddle' /></a>";
	}
	$cal_log->debug('< get_next_cal');
	return $link;
}
...
function getEventList(&$calendar, $start_date, $end_date, $info = '') {
	...
	$group_cond = $search_where.' GROUP BY vtiger_activity.activityid ORDER BY vtiger_activity.date_start,vtiger_activity.time_start ASC';
